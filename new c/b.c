...
// 修改后的反向传播函数，支持Adam优化
void backpropagation(Layer *layers, int num_layers, double **activations, double *target, double learning_rate, double **m, double **v, double **m_hat, double **v_hat, double beta1, double beta2, double epsilon, int t) {
    ...
    // 更新权重和偏置，使用Adam优化
    for (int i = 0; i < num_layers; i++) {
        int size = layers[i].output_size * layers[i].input_size;
        for (int j = 0; j < size; j++) {
            m[i][j] = beta1 * m[i][j] + (1 - beta1) * d_weights[i][j];
            v[i][j] = beta2 * v[i][j] + (1 - beta2) * (d_weights[i][j] * d_weights[i][j]);
            m_hat[i][j] = m[i][j] / (1 - pow(beta1, t));
            v_hat[i][j] = v[i][j] / (1 - pow(beta2, t));
            layers[i].weights[j / layers[i].input_size][j % layers[i].input_size] -= learning_rate * m_hat[i][j] / (sqrt(v_hat[i][j]) + epsilon);
        }
        for (int j = 0; j < layers[i].output_size; j++) {
...
int main() {
    ...
    int num_datasets = sizeof(inputs) / sizeof(inputs[0]);

    double **activations = NULL;
    double learning_rate = 0.001; // 初始学习率
    double beta1 = 0.9; // 动量衰减参数
    double beta2 = 0.999; // RMSprop衰减参数
    double epsilon = 1e-8; // 防止除零的小常数

    // Adam优化算法的变量
    double **m = (double **)malloc(loaded_num_layers * sizeof(double *)); // 一阶矩估计
    double **v = (double **)malloc(loaded_num_layers * sizeof(double *)); // 二阶矩估计
    double **m_hat = (double **)malloc(loaded_num_layers * sizeof(double *)); // 偏差修正后的一阶矩估计
    double **v_hat = (double **)malloc(loaded_num_layers * sizeof(double *)); // 偏差修正后的二阶矩估计

    for (int i = 0; i < loaded_num_layers; i++) {
        int size = layers[i].output_size * layers[i].input_size;
        m[i] = (double *)calloc(size, sizeof(double));
        v[i] = (double *)calloc(size, sizeof(double));
        m_hat[i] = (double *)malloc(size * sizeof(double));
        v_hat[i] = (double *)malloc(size * sizeof(double));
    }
    ...
    for (int epoch = 0; epoch < epoch_num; epoch++) {
        for (int dataset_index = 0; dataset_index < num_datasets; dataset_index++) {
            ...
            backpropagation(layers, loaded_num_layers, activations, targets[dataset_index], learning_rate, m, v, m_hat, v_hat, beta1, beta2, epsilon, epoch + 1);
            // 清理activations内存

...
int main() {
    ...
    double learning_rate = 0.0001; // 调整学习率

    // 训练模型
    for (int epoch = 0; epoch < 2400000; epoch++) {
        ...
        if (epoch % 500 == 0) {
            printf("Epoch %d, Output: %f, %f\n", epoch, activations[loaded_num_layers][0], activations[loaded_num_layers][1]); // 修正这里
        }
    }
